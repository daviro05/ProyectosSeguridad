function upload() {
    ...
    Array.from(ficheros).forEach(function (elemento, index, array) {
        ...
        if(index === array.length-1){
           console.log("Tamaño total de ficheros: "+tam_total);
            procesado = true;
        }
 ...
 function retrieveNewURL(file,ambito,cb) {
  ...
  function uploadFile(file, procesado,url) {
    var xhr = new XMLHttpRequest ()
    xhr.open('PUT', url, true);
    progreso(xhr,procesado,file);
    xhr.send(file);
  }

  function progreso(xhr,procesado,file){
